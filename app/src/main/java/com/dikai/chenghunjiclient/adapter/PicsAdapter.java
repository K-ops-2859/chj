...
public class PicsAdapter  extends RecyclerView.Adapter<PicsAdapter.MyViewHolder> implements View.OnClickListener {

    private Context context;
    private List<HotelAdImg> list;
...
//        holder.mLayout.setOnClickListener(this);
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        HotelAdImg bean = list.get(position);
        Glide.with(context).load(bean.getStartImgUrl()).into(holder.pic);
    }
    ...
    public void addAll(Collection<? extends HotelAdImg> collection){
        int size = list.size();
        list.addAll(collection);
        notifyItemRangeInserted(size, collection.size());
    }

    public void refresh(Collection<? extends HotelAdImg> collection){
        list = new ArrayList<>();
        list.addAll(collection);
