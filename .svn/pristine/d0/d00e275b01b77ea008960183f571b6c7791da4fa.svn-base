...
public class SendCodeActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        mBackLayout = (RelativeLayout) findViewById(R.id.activity_first_back);
        ...
        if(type == Constants.USER_PHONE_REGISTER || type == Constants.USER_NEWS_REGISTER){
            mTitle.setText("注册账号");
    ...
    private void next(ResultCheckCode result){
        if(type == Constants.USER_PHONE_REGISTER){
            ...
            register.setAuthCodeID(result.getAuthCodeID());
            Bundle bundle = new Bundle();
            bundle.putSerializable("register",register);
            startActivity(new Intent(SendCodeActivity.this, SelectIdentActivity.class).putExtras(bundle));
        }else if(type == Constants.USER_NEWS_REGISTER){
            BeanRegister register = new BeanRegister();
            register.setPhoneNo(phone);
            register.setAuthCodeID(result.getAuthCodeID());
            Bundle bundle = new Bundle();
            bundle.putSerializable("register",register);
            startActivity(new Intent(this, CreateNewsActivity.class).putExtras(bundle));
        }else if(type == Constants.USER_PHONE_ACTIVE){
...
//                    .putExtra("phone",phone));
    ...
    private void getAuthCode(){
        ...
        NetWorkUtil.setCallback("User/GetSMSCode",
                new BeanGetAuthCode("1",phone,(type == Constants.USER_PHONE_REGISTER || type == Constants.USER_NEWS_REGISTER)?"0":""+type),
                new NetWorkUtil.CallBackListener() {
                    ...
                    public void onFinish(final String respose) {
                        ...
                        try {
                            ...
                            if ("200".equals(result.getMessage().getCode())) {
                                setfocus();
                                Toast.makeText(SendCodeActivity.this, "验证码已发送", Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(SendCodeActivity.this, result.getMessage().getInform(), Toast.LENGTH_SHORT).show();
                            }
                        } catch (Exception e) {
                    ...
                    public void onError(final String e) {
                        Toast.makeText(SendCodeActivity.this, "网络请求错误", Toast.LENGTH_SHORT).show();
                    }
                });
    }
    ...
    private void inspectCode(){
        ...
        NetWorkUtil.setCallback("User/VerifySMSCode",
                new BeanInspectCode("1",phone,(type == Constants.USER_PHONE_REGISTER || type == Constants.USER_NEWS_REGISTER)?"0":""+type,code),
                new NetWorkUtil.CallBackListener() {
                    @Override
                    public void onFinish(final String respose) {
                        mDialog.dismiss();
